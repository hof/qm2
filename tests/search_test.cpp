...
U64 searchPerft(TSearchData *searchData, int depth, int alpha, int beta) {
    ...
    for (TMove * move = mp->pickFirstMove(searchData, depth, alpha, beta, 0); move;
            move = mp->pickNextMove(searchData, depth, alpha, beta, 0)) {
        if (depth <= 1) {
            result += 1;
        } else {
...
void divide(TSearchData * searchData, int depth) {
    ...
    moveList->clear();
    TMovePicker * mp = searchData->movePicker;
    for (TMove * move = mp->pickFirstMove(searchData, depth, -32000, 32000, 0); move;
            move = mp->pickNextMove(searchData, depth, -32000, 32000, 0)) {
        std::cout << move->asString() << " ";
        searchData->forward(move, pos->givesCheck(move));
...
void testMoveGeneration(string fen, int targetValues[], int maxDepth, THashTable * hashTable) {
    ...
    TSearchData * searchData = new TSearchData(fen.c_str(), PIECE_SQUARE_TABLE, hashTable, &outputHandler);
    ...
    for (int i = 0; i < maxDepth; i++) {
        ...
        string fen_test = searchData->pos->asFen();
