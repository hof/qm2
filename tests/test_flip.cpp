...
bool flip_test(int test_num, const char * fen, int divide_steps = MAX_DIVIDE_STEPS) {
    if (stop_test) {
        return false;
    }
    stop_test = true;
    ...
    /*
     * 1 ply search should (mostly) give equal results
     * Some differences can occur because of different move
     * ordering combined with prune errors. 
     * In that case, the test is refined with a divide
     */
