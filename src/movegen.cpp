...
const int FIRST_PIECE[2] = {BPAWN, WPAWN};

void genEvasions(TBoard * pos, TMoveList * list) {
    genMoves(pos, list);
}

void genQuietChecks(TBoard * pos, TMoveList * list) {
    U64 mask, candidates, moves, target;
    target = ~pos->allPieces;
    TMove * current = list->last;
    list->current = current;
    if (pos->boardFlags->WTM) {
        int kpos = *pos->blackKingPos;
        /*
         * Generate checks by white pawn moves
         */
        if (pos->whitePawns && kpos >= a4) {
            mask = BPawnCaptures[kpos - 8];
            candidates = mask & pos->whitePawns;
            while (candidates) {
                int ssq = popFirst(candidates);
                int tsq = ssq + 8;
                if (pos->Matrix[tsq] == EMPTY) {
                    (current++)->setMove(WPAWN, ssq, tsq);
                }
            }
            if (kpos >= a5 && kpos <= h5) { //check by double pawn push
                mask = BPawnCaptures[kpos - 16];
                candidates = mask & pos->whitePawns;
                while (candidates) {
                    int ssq = popFirst(candidates);
                    int tsq = ssq + 16;
                    if (pos->Matrix[tsq] == EMPTY && pos->Matrix[ssq + 8] == EMPTY) {
                        (current++)->setMove(WPAWN, ssq, tsq);
                    }
                }
            }
        ...
        /*
         * Generate checks by white knights
         */
        if (pos->whiteKnights) {
            mask = KnightMoves[kpos] & target;
            for (int i = 0; i < pos->pieces[WKNIGHT].count; i++) {
                int ssq = pos->pieces[WKNIGHT].squares[i];
                moves = KnightMoves[ssq] & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(WKNIGHT, ssq, tsq);
                }
            }
        }
        /*
         * Generate checks by white bishops
         */
        if (pos->whiteBishops) {
            mask = MagicBishopMoves(kpos, pos->allPieces) & target;
            for (int i = 0; i < pos->pieces[WBISHOP].count; i++) {
                int ssq = pos->pieces[WBISHOP].squares[i];
                moves = MagicBishopMoves(ssq, pos->allPieces) & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(WBISHOP, ssq, tsq);
                }
            }
        }
        /*
         * Generate checks by white rooks
         */
        if (pos->whiteRooks) {
            mask = MagicRookMoves(kpos, pos->allPieces) & target;
            for (int i = 0; i < pos->pieces[WROOK].count; i++) {
                int ssq = pos->pieces[WROOK].squares[i];
                moves = MagicRookMoves(ssq, pos->allPieces) & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(WROOK, ssq, tsq);
                }
            }
        }
        /*
         * Generate checks by white queens
         */
        if (pos->whiteQueens) {
            mask = MagicQueenMoves(kpos, pos->allPieces) & target;
            for (int i = 0; i < pos->pieces[WQUEEN].count; i++) {
                int ssq = pos->pieces[WQUEEN].squares[i];
                moves = MagicQueenMoves(ssq, pos->allPieces) & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(WQUEEN, ssq, tsq);
                }
            }
        }
        ...
        if (kpos <= h5 && pos->blackPawns) {
            mask = WPawnCaptures[kpos + 8];
            /*
             * Generate checks by black pawn moves
             */
            candidates = mask & pos->blackPawns;
            while (candidates) {
                int ssq = popFirst(candidates);
                int tsq = ssq - 8;
                if (pos->Matrix[tsq] == EMPTY) {
            ...
            if (kpos >= a6 && kpos <= h6) { //check by double pawn push
                mask = BPawnCaptures[kpos + 16];
                candidates = mask & pos->blackPawns;
                while (candidates) {
                    int ssq = popFirst(candidates);
        ...
        /*
         * Generate checks by black knights
         */
        if (pos->blackKnights) {
            mask = KnightMoves[kpos] & target;
            for (int i = 0; i < pos->pieces[BKNIGHT].count; i++) {
                int ssq = pos->pieces[BKNIGHT].squares[i];
                moves = KnightMoves[ssq] & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(BKNIGHT, ssq, tsq);
                }
            }
        }
        /*
         * Generate checks by black bishops
         */
        if (pos->blackBishops) {
            mask = MagicBishopMoves(kpos, pos->allPieces) & target;
            for (int i = 0; i < pos->pieces[BBISHOP].count; i++) {
                int ssq = pos->pieces[BBISHOP].squares[i];
                moves = MagicBishopMoves(ssq, pos->allPieces) & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(BBISHOP, ssq, tsq);
                }
            }

        }
        /*
         * Generate checks by black rooks 
         */
        if (pos->blackRooks) {
            mask = MagicRookMoves(kpos, pos->allPieces) & target;
            for (int i = 0; i < pos->pieces[BROOK].count; i++) {
                int ssq = pos->pieces[BROOK].squares[i];
                moves = MagicRookMoves(ssq, pos->allPieces) & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(BROOK, ssq, tsq);
                }
            }

        }
        /*
         * Generate checks by black queens
         */
        if (pos->blackQueens) {
            mask = MagicQueenMoves(kpos, pos->allPieces) & target;
            for (int i = 0; i < pos->pieces[BQUEEN].count; i++) {
                int ssq = pos->pieces[BQUEEN].squares[i];
                moves = MagicQueenMoves(ssq, pos->allPieces) & mask;
                while (moves) {
                    int tsq = popFirst(moves);
                    (current++)->setMove(BQUEEN, ssq, tsq);
                }
            }
        }
    }
